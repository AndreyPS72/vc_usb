...
resourcestring
      ...
      ErrorVersionUp             = 'Версия протокола обмена %s не поддерживается данной программы.'+#10+#13+'Пожалуйста обновите программу';
  ...
  RouteDownloadErrorProtokol        = 'Данная функция реализована только для приборов Диана-8, Диана-2М,'+#10+#13+'Диана-С, Vibro Vision-2, Viana-1, Viana-4 и Вик-3.'+#10+#13+'Для того чтобы скачать маршрут выберите соответствующий прибор.';
  ...
  RouteUploadErrorProtokol       = 'Данная функция реализована только для приборов Диана-8, Диана2М, Диана-С, Vibro Vision-2 и Вик-3.'+#10+#13+'Для того чтобы закачать маршрут выберите соответствующий прибор.';
  RouteUploadLoadCaptionRoute    = ' Загрузка заголовка маршрута ';
  RouteUploadRouteExist          = 'Маршрут с именем "%s" уже существует в приборе.'+#10#13+'Желаете его перезаписать?';
