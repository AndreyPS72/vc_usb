...
procedure RealFFT_WindowHamming(const re: PReal64ArrayZeroBased; var N: integer); // с Окном Хемминга
...
procedure RealFFT_WindowHamming(const re: PReal64ArrayZeroBased; var N: integer);
var count, i: integer;
    scale: double;
begin
  ...
  for i:=0 to count-1 do begin
      re^[i]:=re^[i]*(0.53836-0.46164*cos(2*Pi*i/(count-1))); // Окно Хемминга
      im[i]:=0;
  end;
  DoFFT(re, @im, count);

  scale:=2.0/double(count)*1.8556668;
  for i:=0 to count div 2 do begin
      re^[i]:= sqrt(sqr(re^[i])+sqr(im[i]))*scale;
  end;
  N:=(count div 2)+1;
